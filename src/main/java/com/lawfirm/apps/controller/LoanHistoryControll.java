...
public class LoanHistoryControll {
    ...
    @RequestMapping(value = "/loan-a-history/view/by-admin", method = RequestMethod.GET, produces = {"application/json"})///{id_employee}
    @XxsFilter
    public ResponseEntity<?> laonAbyAdmin(Long id_employee, Authentication authentication) { //find by admin id
        try {
            ...
            if (entity == null) {
                ...
                CreateLog.createJson(rs, "loan-a-history-view-by-admin");
...
//            e.printStackTrace();
    ...
    @RequestMapping(value = "/loan-a-history/view/by-employee", method = RequestMethod.GET, produces = {"application/json"})///{id_employee}
    @XxsFilter
    public ResponseEntity<?> laonAbyEmployee(Long id_employee, Authentication authentication) { //find by employee id
        try {
            String name = authentication.getName();
            log.info("name : " + name);
            Employee entity = employeeService.findByEmployee(name);
            ...
            if (entity == null) {
                ...
                rs.setInfo("Failed");
                rs.setResponse("can't acces this feature :");
                CreateLog.createJson(rs, "loan-a-history-view-by-employee");
                return new ResponseEntity(new CustomErrorType("55", "Error", "can't acces this feature"),
                        HttpStatus.NOT_FOUND);
            }
            if (entity.getRoleName().contains("admin")) {
...
//            e.printStackTrace();
    ...
    @RequestMapping(value = "/loan-b-history/view/by-admin", method = RequestMethod.GET, produces = {"application/json"})///{id_employee}
    @XxsFilter
    public ResponseEntity<?> laonbByAdmin(Long id_employee, Authentication authentication) { //find by admin id
        try {
            String name = authentication.getName();
            log.info("name : " + name);
            Employee entity = employeeService.findByEmployee(name);
            ...
            if (entity == null) {
                ...
                rs.setInfo("Failed");
                rs.setResponse("can't acces this feature :");
                CreateLog.createJson(rs, "loan-b-history-view-by-admin");
                return new ResponseEntity(new CustomErrorType("55", "Error", "can't acces this feature"),
                        HttpStatus.NOT_FOUND);
            }
            if (entity.getRoleName().contains("admin")) {
...
//            e.printStackTrace();
    ...
    @RequestMapping(value = "/loan-b-history/view/by-employee", method = RequestMethod.GET, produces = {"application/json"})///{id_employee}
    @XxsFilter
    public ResponseEntity<?> laonbByEmployee(Long id_employee, Authentication authentication) { //find by employee id
        try {
            String name = authentication.getName();
            log.info("name : " + name);
            Employee entity = employeeService.findByEmployee(name);
            ...
            if (entity == null) {
                ...
                rs.setInfo("Failed");
                rs.setResponse("can't acces this feature :");
                CreateLog.createJson(rs, "loan-b-history-view-by-employee");
                return new ResponseEntity(new CustomErrorType("55", "Error", "can't acces this feature"),
                        HttpStatus.NOT_FOUND);
            }
            if (entity.getRoleName().contains("admin")) {
