...
//            e.printStackTrace();
    ...
    @XxsFilter
    public ResponseEntity<?> DisburseByEmployee(@RequestBody final DisbursementCaseIdDto object, Authentication authentication) {
        try {
            Date now = new Date();
            ...
            if (entityEmp == null) {
                ...
                rs.setInfo("Failed");
                rs.setResponse("can't acces this feature :");
                log.error("DisburseByEmployee" + rs.toString());
...
//                                
            ...
            CreateLog.createJson(ex.getMessage(), "DisburseByEmployee");
            return new ResponseEntity(new CustomErrorType("55", "Error", "can't acces this feature case Id : " + object.getCase_id() + " Not Found"),
                    HttpStatus.NOT_FOUND);
        } catch (ParseException ex) {
            log.error("DisburseByEmployee : " + ex.getMessage());
