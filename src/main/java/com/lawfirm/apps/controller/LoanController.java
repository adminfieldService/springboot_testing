...
//    Date now;
    ...
    public LoanController() {
        ...
        this.sdfYear2 = new SimpleDateFormat("yyyy");
    ...
    public Response createLoana(@RequestBody final LoanApi object, Authentication authentication) {
        try {
            ...
            log.info("createLoana jsonObject: " + object);
            if (entityEmp == null) {
...
//            Financial dataFinance = new Financial();
            ...
            if ("A".equalsIgnoreCase(object.getLoan_type())) {
                ...
                if (process) {
                    ...
                    System.out.println("isi process " + process);
                    ...
                    if (tgl_repayment.compareTo(tgl_pengajuan) < 0) {
                        rs.setResponse_code("55");
                        rs.setInfo("Failed");
                        rs.setResponse("tgl_pengajuan > tgl_repayment");
                        process = false;
                        CreateLog.createJson(rs, "createLoana");
                        log.error("createLoana : " + rs.toString());
                        return rs;
                    ...
                    if (tgl_repayment.compareTo(tgl_pengajuan) == 0) {
                        rs.setResponse_code("55");
                        rs.setInfo("Failed");
                        rs.setResponse("tgl repayment == tgl_pengajuan");
                        process = false;
                        CreateLog.createJson(rs, "createLoana");
                        log.error("createLoana : " + rs.toString());
                        return rs;
                    ...
                    dataLoan.setTgl_input(sdfYear2.format(now));
...
//                    dataLoan.setTahunInput(sdfYear.format(now));
                    ...
                    if (entityEmp.getLoanAmount().compareTo(object.getLoan_amount()) < 0) {
                        rs.setResponse_code("55");
                        rs.setInfo("Failed");
                        rs.setResponse("limit loan : " + String.format("%.0f", entityEmp.getLoanAmount()));
                        process = false;
                        CreateLog.createJson(rs, "createLoana");
                        log.error("createLoana : " + rs.toString());
                        return rs;
...
//                        }
//                    } else {
//                        List<Loan> list = loanService.generateLoanId("A", entityEmp.getEmployeeId(), sdfYear2.format(object.getCreated_date()));
//
//                        if (list != null || !list.isEmpty()) {
//                            number = list.size();
//                        }
...
//                   typeLoan.addLoan(dataLoan);
                    if (dataLoan != null) {
                        ...
                        CreateLog.createJson(rs, "createLoana");
                        log.info("createLoana : " + rs.toString());
                        return rs;
//                        }
                    } else {
                        rs.setResponse_code("55");
            ...
            CreateLog.createJson(rs, "createLoana");
            log.error("createLoana : " + ex.getMessage());
            return rs;
        }
    ...
    public Response createLoanb(@RequestBody final LoanApi object, Authentication authentication) {
        try {
            log.info("createLoanb jsonObject: " + object);
            Date todayDate = new Date();
            Date now = new Date();
            String name = authentication.getName();
            log.info("name : " + name);
            ...
            if (entityEmp == null) {
                ...
                log.error("createLoanb" + rs.toString());
                return rs;
            }
            log.info("isi object " + object.toString());
            Boolean process = true;
...
//                CaseDetails dataCase = new CaseDetails();
                ...
                if (process) {
                    ...
                    Double dmpPortion = dataCase.getDmpPortion();
                    ...
                    if (dataCase.getStatus().contains("s")) {
                        rs.setResponse_code("55");
                        rs.setInfo("Failed");
                        rs.setResponse("CASE ID : " + dataCase.getCaseID() + " Need Approval by Admin ");
                        process = false;
                        log.error("createLoanb" + rs.toString());
                        CreateLog.createJson(rs, "createLoanb");
                        return rs;
                    }
                    if (dataCase.getStatus().contains("closed")) {
                        rs.setResponse_code("55");
                        rs.setInfo("Failed");
                        rs.setResponse("CASE ID : " + dataCase.getCaseID() + " Status closed ");
                        process = false;
                        log.error("createLoanb" + rs.toString());
                        CreateLog.createJson(rs, "createLoanb");
                        return rs;
...
//                    Integer number = 0;
                    ...
                    dataLoan.setTgl_input(sdfYear2.format(now));
                    dataLoan.setRepayment_date(tgl_repayment);
                    log.info("dataCase.getEngagementId()" + dataCase.getEngagementId());
                    Double cek_loan = this.loanService.loanB(dataCase.getEngagementId());
                    Double balance = 0.0;
                    if (cek_loan != null || !cek_loan.equals(0)) {
                        balance = object.getLoan_amount() + cek_loan;
                    } else {
...
//                    }
                    ...
                    if (dmpPortion.compareTo(balance) < 0) {
                        rs.setResponse_code("55");
                        rs.setInfo("Failed");
                        rs.setResponse("limit loan B CASE ID" + dataCase.getCaseID() + " : " + String.format("%.0f", dmpPortion) + " Total Loan : " + String.format("%.0f", balance));
                        process = false;
                        CreateLog.createJson(rs, "createLoanb");
                        log.error("createLoanb : " + rs.toString());
                        return rs;
...
//                            log.info("isi dFinancial " + dFinancial.getFinancialId().toString());
                        rs.setResponse_code("01");
                        rs.setInfo("Success");
                        rs.setResponse("Create Loan Type B  Succes");
//                        }
                    }
//                    rs.setResponse_code("55");
//                    rs.setInfo("Failed");
//                    rs.setResponse("Create Loan Apps Type B Failed");
            ...
            rs.setInfo("Failed");
            rs.setResponse(ex.getMessage());
            log.error("createLoanb" + ex.getMessage());
            CreateLog.createJson(rs, "createLoan-employee");
            return rs;
        }
...
//
            if (entityEmp == null) {
                ...
                log.error("loan-approve-ByAdmin : " + rs.toString());
...
//                process = false;
            ...
            if (!entityEmp.getRoleName().contentEquals("admin")) {
                ...
                log.error("loan-approve-Byfinance : " + rs.toString());
                CreateLog.createJson(rs, "loan-approve-ByAdmin");
//                process = false;
                return rs;
            }
//             Employee dataEMploye = employeeService.findById(object.getId_employee_admin());
            ...
            if (dataLoan.getIsActive().contentEquals("0") || dataLoan.getStatus().contentEquals("a")) {
                Employee dataAdmin = this.employeeService.findById(Long.parseLong(dataLoan.getAprovedByAdmin()));
                rs.setResponse_code("55");
                rs.setInfo("Failed");
                rs.setResponse("Loan Already Approved, at : " + dataAdmin.getEmployeeId());
                CreateLog.createJson(rs, "loan-approve-ByAdmin");
                log.error("loan-approve-ByAdmin : " + rs.toString());
                process = false;
                return rs;
            ...
            if (dataLoan.getIsActive().contentEquals("2") || dataLoan.getStatus().contentEquals("r")) {
                ...
                rs.setResponse_code("55");
                rs.setInfo("Failed");
                rs.setResponse("Loans Rejected, at : " + dataAdmin.getEmployeeId());
                CreateLog.createJson(rs, "loan-approve-ByAdmin");
                log.error("loan-approve-ByAdmin : " + rs.toString());
                process = false;
                return rs;
            ...
            if (process) {
                ...
                if (object.getDecision().contains("r")) {
                    ...
                    if (dataLoan.getLoantype().getTypeLoan().contentEquals("a")) {
                        Employee employee = employeeService.findById(dataLoan.getEmployee().getIdEmployee());
                        String year_val = sdfYear.format(now);
                        Integer number = 0;
                        Date y_val = sdfYear.parse(year_val);
                        String load_id = null;
                        ...
                        String year = sdfYear2.format(now);
                        if (year.compareTo(dataLoan.getTgl_input()) == 0) {
                            List<Loan> list = loanService.generateLoanId("A", employee.getEmployeeId(), dataLoan.getTgl_input());
                            ...
                            load_id = "A" + employee.getEmployeeId() + year_val + Util.setNumbering(number.toString());
//                            switch (number) {
//                                case 0:
//                                    number = 1;
//                                    load_id = "A" + employee.getEmployeeId() + year_val + Util.setNumbering(number.toString());
...
//                            }
                        ...
                        System.out.println("load_id now : " + load_id);
                        dataLoan.setLoanId(load_id);
                        entityHistory.setResponse("approve");
                    }
                    if (dataLoan.getLoantype().getTypeLoan().contentEquals("b")) {
                        Integer number = 0;
                        String load_id = null;
                        Engagement engagement = engagementService.findById(dataLoan.getEngagement().getEngagementId());
                        if (engagement == null) {
                            rs.setResponse_code("55");
                            rs.setInfo("Failed");
                            rs.setResponse("Engagement Id Not Found");
                            log.error("loan-approve-ByAdmin : " + rs.toString());
                            CreateLog.createJson(rs, "loan-approve-ByAdmin ");
                            return rs;
                        }
                        String caseId = engagement.getCaseID();
                        List<Loan> list = loanService.generateLoanIdB(caseId, null, null);
                        if (list != null || !list.isEmpty()) {
                            number = list.size();
                        } else {
                            number = 0;
                        }
                        load_id = "BCS" + Util.removeCase(caseId) + Util.setNumbering(number.toString());
//                        switch (number) {
//                            case 0:
//                                number = 1;
//                                load_id = "BCS" + Util.removeCase(caseId) + Util.setNumbering(number.toString());
...
//              dataLoan.addAHistory(entityHistory);
                ...
                if (upDataLoan.getIsActive().contentEquals("0")) {
                    ...
                    rs.setInfo("Success");
                    rs.setResponse("Loan apps : " + entityEmp.getName() + " Approved");
                    CreateLog.createJson(rs, "loan-approve-ByAdmin");
                    log.info("loan-approve-ByAdmin : " + rs.toString());
                }
                if (upDataLoan.getIsActive().contentEquals("2")) {
                    rs.setResponse_code("00");
                    rs.setInfo("Success");
                    rs.setResponse("Loan apps : " + entityEmp.getName() + " Rejected");
                    CreateLog.createJson(rs, "loan-approve-ByAdmin");
                    log.info("loan-approve-ByAdmin : " + rs.toString());
                }
//            else {
...
//            }
            ...
            log.error("loan-approve-ByAdmin : " + ex.getMessage());
            return rs;
...
//
            if (entityEmp == null) {
                ...
                rs.setInfo("Failed");
                rs.setResponse("can't acces this feature :");
                CreateLog.createJson(rs, "approveLoanBByAdmin");
                log.error("approveLoanBByAdmin : " + rs.toString());
//                process = false;
                return rs;
            }
            if (!entityEmp.getRoleName().contentEquals("admin")) {
...
//        dataLoan.addAHistory(entityHistory);
                ...
                if (upDataLoan.getIsActive().contentEquals("0")) {
                    ...
                    log.info("approveLoanBByAdmin : " + rs.toString());
                    CreateLog.createJson(rs, "loan-approve-ByAdmin");
                }
                if (upDataLoan.getIsActive().contentEquals("2")) {
                    rs.setResponse_code("00");
...
//            }
            ...
            rs.setInfo("Failed");
            rs.setResponse(ex.getMessage());
            log.error("approveLoanBByAdmin : " + ex.getMessage());
            CreateLog.createJson(rs, "loan-approve-ByAdmin");
            CreateLog.createJson(ex.getMessage(), "loan-approve-ByAdmin");
            return rs;
...
//                HttpStatus.NOT_FOUND);
    ...
    public Response approveByFinance(@RequestBody final LoanApi object, @PathVariable("id_loan") Long id_loan, Authentication authentication) {
        try {
            ...
            Employee entityEmp = employeeService.findByEmployee(name);
            log.info("entity : " + entityEmp);
            log.info("approveByFinance JSON : " + object);
...
//        }
            if (entityEmp == null) {
                ...
                rs.setResponse("can't acces this feature :");
                CreateLog.createJson(rs, "loan-approve-Byfinance");
                log.error("loan-approve-Byfinance JSON : " + rs.toString());
//                process = false;
                return rs;
            }
...
//                    }
                    ...
                    rs.setResponse("Loan apps Approved By :" + dataLoan.getAprovedByFinance());
                    CreateLog.createJson(rs, "loan-approve-Byfinance");
                    log.info("loan-approve-Byfinance JSON : " + rs.toString());
                    return rs;
                } else {
                    rs.setResponse_code("55");
                    rs.setInfo("Failed");
            ...
            log.error("loan-approve-Byfinance  : " + ex.getMessage());
            return rs;
    ...
    public ResponseEntity<String> viewLoan(Authentication authentication, HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            if (entityEmp == null) {
                ...
                rs.setInfo("Failed");
                rs.setResponse("can't acces this feature :");
                CreateLog.createJson(rs, "view-Loan-a");
//                process = false;
                log.error("view-Loan-a  : " + rs.toString());
...
//            CreateLog.createJson(rs, "list-of-loan-a");
            ...
            log.error("view-Loan-a  : " + ex.getMessage());
...
//            int max = loanService.count();
            ...
            if (start == 0) {
                entityList = this.loanService.listBy(dataEmp.getIdEmployee().toString(), param, "a");
            } else {
                entityList = this.loanService.listBy(dataEmp.getIdEmployee().toString(), param, "a");
            }
...
//            Employee dataEmp = employeeService.findById(id_employee_admin);
            ...
            if (start == 0) {
                entityList = this.loanService.listBy(dataEmp.getIdEmployee().toString(), param, "b");
            } else {
                entityList = this.loanService.listBy(dataEmp.getIdEmployee().toString(), param, "b");
            }
