...
//  .exceptionHandling().authenticationEntryPoint(new AuthExceptionEntryPoint()).and() 
        http.cors().and().csrf().csrfTokenRepository(CookieCsrfTokenRepository.withHttpOnlyFalse()).disable()
                ...
                .antMatchers(HttpMethod.POST, "/engagement/manage-engagement/{engagement_id}/").hasRole("dmp")
                ...
                .antMatchers(HttpMethod.POST, "/disbursement-dto/case-id/").hasRole("finance")
                .antMatchers(HttpMethod.POST, "/disbursement-dto/employee/").hasRole("finance")
