...
//  .exceptionHandling().authenticationEntryPoint(new AuthExceptionEntryPoint()).and() 
        http.cors().and().csrf().csrfTokenRepository(CookieCsrfTokenRepository.withHttpOnlyFalse()).disable()
                ...
                .antMatchers(HttpMethod.POST, "/employee/managed-employee/set-password/").hasAnyRole("lawyer", "admin", "dmp", "support")//.permitAll()       
                ...
                .antMatchers(HttpMethod.POST, "/disburse/{id_loan}/").hasRole("finance")
